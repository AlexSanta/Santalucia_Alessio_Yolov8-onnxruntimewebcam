...
export const detectImage = async (
  ...
  renderBoxes(canvas, boxes, image); // Draw boxes
  input.delete(); // delete unused Mat

  //if at least one object has been detected, communicate with server
  if (boxes.length > 0){
    // Create a new FormData object
    const formData = new FormData();

    // Convert the boxes data to a JSON string and append it to the FormData object
    formData.append('boxes', JSON.stringify(boxes));

    // Get the current timestamp
    const currentTime = new Date();
    currentTime.setHours(currentTime.getHours() + 2);
    const frameTimestamp = currentTime.toISOString();

    // Append the timestamp to the FormData object
    formData.append('frameTimestamp', frameTimestamp);
    ...
    // Send the image and boxes data to the Flask server
    fetch('/detected', {
      method: 'POST',
      body: formData
    })
